...
export default function ContentResult({ content }: { content: string }) {
  ...
  // Split content into variations if separator exists
  const variations = content.split('---VARIATION SEPARATOR---').map(v => v.trim());
        ...
        <div key={index} className="bg-gray-50 rounded-lg border border-gray-200 p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-gray-900">
              {variations.length > 1 ? `Variation ${index + 1}` : 'Generated Content'}
            </h3>
            <button
              onClick={() => handleCopy()}
          ...
          <div className="prose max-w-none">
            <div className="whitespace-pre-wrap text-gray-800 font-medium">{variation}</div>
          </div>
        </div>
      ))}
    </div>
